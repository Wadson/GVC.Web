...
public class IndexModel(ErpDbContext db) : BasePageModel
{
    ...
    {
        if (await db.Caixas.AnyAsync(x => x.EmpresaId == EmpresaId && x.Status == "Aberto"))
        {
    ...
    {
        var caixa = await db.Caixas.SingleOrDefaultAsync(x => x.EmpresaId == EmpresaId && x.Status == "Aberto");

        if (caixa is null)
            return RedirectToPage();

        var mov = await db.CaixaMovimentos.Where(x => x.CaixaId == caixa.CaixaId).SumAsync(x => (decimal?)(x.Tipo == "ENTRADA" ? x.Valor : -x.Valor)) ?? 0;
    ...
    {
        CaixaAtual = await db.Caixas.AsNoTracking().SingleOrDefaultAsync(x => x.EmpresaId == EmpresaId && x.Status == "Aberto");

        if (CaixaAtual is not null)
            Saldo = CaixaAtual.SaldoInicial + (await db.CaixaMovimentos.Where(x => x.CaixaId == CaixaAtual.CaixaId).SumAsync(x => (decimal?)(x.Tipo == "ENTRADA" ? x.Valor : -x.Valor)) ?? 0);
    }
}